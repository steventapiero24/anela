...
const CalendarView = ({ setStep, cart, removeFromCart, CATEGORIES, selectedDate, setSelectedDate, selectedTime, setSelectedTime, user, saveAppointment, reschedulingId }) => {
  ...
  // 🚀 GENERADOR DE FECHAS DINÁMICAS (Para no usar un array estático)
  // 🚀 GENERADOR DE FECHAS DINÁMICAS CORREGIDO
  const generateDays = () => {
    const days = [];
    ...
    for (let i = 0; i < 7; i++) {
      // Creamos una nueva fecha basada en el momento exacto de hoy
      const date = new Date();
      
      // Le sumamos los días (0 para hoy, 1 para mañana, etc.)
      date.setDate(date.getDate() + i);
      
      const dayNum = date.getDate();
      const monthStr = months[date.getMonth()];
      
      // Formato idéntico: "5 Abr", "6 Abr", etc.
      days.push(`${dayNum} ${monthStr}`);
    }
    return days;
  };
    ...
    <div className="space-y-8 animate-in">
      ...
      <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-gray-100">
         ...
         <div className="flex gap-4 overflow-x-auto pb-6 scrollbar-hide">
           {/* 🌟 AQUÍ USAMOS LAS FECHAS DINÁMICAS */}
           {dynamicDays.map(d => (

...
import { Calendar as CalendarIcon, RefreshCw, X, Star, Plus, Check, Scissors, Sparkles, Flower2, Zap } from 'lucide-react';
...
const HomeView = ({ appointments, setStep, CATEGORIES, SERVICES, cart, addToCart, startReschedule, cancelAppointment, setSelectedCategory }) => (
  <div className="space-y-8">
            ...
            <div key={appt.id} className="min-w-[300px] bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-5">
              <div className="flex justify-between items-start border-b border-gray-50 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                      <h4 className="font-black text-sm text-primary">{appt.date}</h4>
              ...
              <div className="flex gap-2 pt-2">
                <button 
                  ...
                  className="flex-1 py-3.5 bg-gray-50 text-primary rounded-2xl text-[10px] font-black flex items-center justify-center gap-2 hover:bg-gray-100 active:scale-95 transition-all uppercase tracking-widest"
                >
    ...
    <section>
      ...
      <div className="flex justify-between overflow-x-auto scrollbar-hide gap-4">
        {CATEGORIES.map(cat => (
          <button key={cat.id} onClick={() => {setSelectedCategory(cat); setStep('services')}} className="flex flex-col items-center gap-3 group">
            <div className={`w-16 h-16 ${cat.bg} rounded-full flex items-center justify-center text-primary shadow-sm group-active:scale-90 transition-transform`}>
              {cat.icon}
            </div>
    ...
    <section className="space-y-5">
      <div className="flex justify-between items-center">
        ...
        <button className="text-[11px] font-bold text-primary">Ver todos</button>
      </div>
      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
        {SERVICES.filter(s => s.recommended).map(s => (
          <div key={s.id} className="min-w-[200px] bg-white rounded-[2rem] p-4 shadow-sm border border-gray-50 space-y-3">
            <div className="relative">
               ...
               <span className="absolute top-2 right-2 bg-white/90 backdrop-blur-md px-2 py-1 rounded-lg text-[9px] font-bold text-primary">★ {s.rating}</span>
            </div>
            <div>
            ...
            <div className="flex justify-between items-center">
              ...
              <button 
                onClick={() => addToCart(s)} 
                className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${cart.find(i => i.id === s.id) ? 'bg-primary-light text-primary' : 'bg-gray-50 text-gray-400'}`}
              >
